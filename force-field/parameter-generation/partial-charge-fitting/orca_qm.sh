...
tolerance='ExtremeSCF' # SCF convergence tolerance {TightSCF, VeryTightSCF, ExtremeSCF}
...
input_dir="${input_base_dir}/monomers"
...
cp "${input_dir}/${job}.pdb" "${job}.pdb"
obabel "${job}.pdb" -O "${job}.xyz" 2>&1 | tee -a "obabel.log"
# copy the equivalence constraint file over
cp "${input_dir}/${job}-eqvcons.txt" "eqvcons.txt"
cp "${input_dir}/${job}-chgcons.txt" "chgcons.txt"
